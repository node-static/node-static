...
class Server extends events.EventEmitter {
    ...
    serveFile (pathname, status, headers, req, res) {
        ...
        tryStat(pathname, (e, stat) => {
            if (e || !stat) {
                // Mirror `finish()`: only emit `error` when something is
                //   listening for it. A caller that ignores the returned
                //   emitter (e.g. the CLI, or the `serveFile` examples in the
                //   README) would otherwise get an unhandled `'error'` event,
                //   crashing the process, when the file is missing. Fall back
                //   to a 404 response in that case.
     ...
     *   req: http.IncomingMessage;
     * }} res
     * @param {events.EventEmitter} promise
